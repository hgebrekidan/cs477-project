...
function renderProductMember(prod) {
    ...
    updateBtn.addEventListener('click', function(event) {
        ...
        // document.getElementById('qty').value=0;
          document.getElementById('libraryID').value = Number(document.getElementById('libraryID').value) + Number(isbn.textContent) //* document.getElementById('qty').value;
          document.getElementById('qty').value=Number(document.getElementById('qty').value)+1;

           document.getElementById('displayArea').value = document.getElementById('displayArea').value +""+  h2.textContent +"\n"+ document.getElementById('libraryId').value+"\n"+ document.getElementById('qty').value;;
            
           document.getElementById('libraryID').value.reset();
 let data= await (await(fetch("http://localhost:3000/books/"+prod.id)).json() )
          let username=sessionStorage.getItem('username');
            ...
            window.onload.innerHTML = data;
            
            const title = document.createElement('h2');
    title.textContent = data.title;

    const price = document.createElement('p');
    price.textContent = Number(data.price);

    const booksName = document.createElement('p');
    booksName.textContent = data.booksName;
  ...
  //show cart 
//   let data= await (await(fetch("/carts/"+sessionStorage.getItem('username'))).json() )
//     //  data.forEach(d=>{
//     //     //  displayCart(d)
//     //  })
...
//  }
    ...
    deleteBtn.addEventListener('click', function(event) {
        ...
        if(Number(document.getElementById('libraryID').value)<0){
            document.getElementById('libraryID').value=0; 
        }else{
            document.getElementById('libraryID').value = Number(document.getElementById('libraryID').value) - Number(isbn.textContent) //* document.getElementById('qty').value;
            document.getElementById('qty').value=Number(document.getElementById('qty').value)-1;
              let data= await (await(fetch("/carts/"+sessionStorage.getItem('username'),{
                method:"DELETE"
            })).json() )
     data.forEach(d=>{
         displayCart(d)
...
function editProduct() {
    ...
    fetch('http://localhost:3000/books/' + prodId, {
        ...
        document.getElementById('product-btn2').addEventListener('onclick', checkoutPage);
        function checkoutPage(){
            alert('Purchased Successfully!!')
            
        }
// document.getElementById('submitDetails').onclick = function(event){
//     event.preventDefault();
//     alert('signed up successfully!!')
//     document.getElementById('div-member').style.display = 'none'
//     let result = fetch('http://localhost:3000/signup', {
//         method: "POST",
