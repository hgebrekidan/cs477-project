...
window.onload = function() {
    ...
    document.getElementById('loginBtn').onclick = async function(event) {
        ...
        let result = await fetch('http://localhost:3222/login', {
            method: 'POST',
            headers: {
...
async function getProducts() {
    let products = await fetch('http://localhost:3222/books/', {
        method: 'GET',
        headers: {
            'Content-Type':'application/json',
...
function renderProduct(prod) {
    ...
    deleteBtn.addEventListener('click', function(event) {
        ...
        fetch('http://localhost:3222/books/' + prod._id, {
            method: 'DELETE',
            headers: {
                'Authorization': 'Bearer ' + sessionStorage.getItem('accessToken')
            }
...
async function addProduct() {
    let result = await fetch('http://localhost:3222/books/', {
        method: 'POST',
        headers: {
            'Content-type': 'application/json',
            'Authorization': 'Bearer ' + sessionStorage.getItem('accessToken')
